...
for title, bullets, col, x in s1_data:
    ...
    # 2. Downward Pointed Header Banner
    pts_banner = [
...
for title, bullets, col, y in s3_data:
    ...
    fld_pts = [(Inches(2.00), Inches(y + 0.15)), (Inches(2.90), Inches(y)), (Inches(2.90), Inches(y + 1.60)), (Inches(2.00), Inches(y + 1.45))]
    ...
    # 2. Right-pointing Chevron Body (Starts directly connected)
    chv = s3.shapes.add_shape(MSO_SHAPE.CHEVRON, Inches(2.20), Inches(y), Inches(5.80), Inches(1.60))
    chv.fill.solid()
    ...
    tf_chv.margin_left = Inches(1.00)
    ...
    disc_outer = s3.shapes.add_shape(MSO_SHAPE.OVAL, Inches(0.85), Inches(y - 0.10), Inches(1.80), Inches(1.80))
    disc_outer.fill.solid()
    disc_outer.fill.fore_color.rgb = hex_to_rgb('#FFFFFF')
    disc_outer.line.fill.background()
    
    disc = s3.shapes.add_shape(MSO_SHAPE.OVAL, Inches(0.95), Inches(y), Inches(1.60), Inches(1.60))
    disc.fill.solid()
...
# Pass 1: Draw all 5 circles and text
...
for title, desc, col, num, x in s4_data:
    c_rgb = hex_to_rgb(col)
    cres = s4.shapes.add_shape(MSO_SHAPE.OVAL, Inches(x + diam_v - 0.70), Inches(y_v + diam_v/2 - 0.50), Inches(0.85), Inches(1.00))
    cres.fill.solid()
...
print('Saved Folder 2 perfected presentation to:', out_file)
